/*
Code for buffer moved here. This is also I/O related code.
This is modified to support multiple I/Os.
Packet code is taken from Qualcomm's ARMPRG packet handling source.
*/
#include "dcc/dn_dcc_proto.h"
#include "io/io_type.h"
#include <stdint.h>

/* 00 - Shared */
#include <memory.h> 

#ifdef PLATFORM_USES_USB
extern drl_io_funcs_t io_usb;
#endif
#ifdef PLATFORM_USES_UART
extern drl_io_funcs_t io_uart;
#endif

drl_io_funcs_t* DRL_Current_IO = NULL;

/* I/O stuffs */
bool DRL_IO_Setup(void)
{
    bool ret = false;
#ifdef PLATFORM_USES_USB
    // We need to ensure the USB is used for dl'ing this binary
    if (io_usb.init())
    {
        //wait till active...
        while (io_usb.active() == false)
        {
            ;
        }
        DRL_Current_IO = &io_usb;
        return true;
    }
#endif
#ifdef PLATFORM_USES_UART
    //either no USB, or build has no USB support
    io_uart.init();
    DRL_Current_IO = &io_uart;
    ret = true;
#endif
    return ret;
}

/* CRC */

#define CRC_16_L_POLYNOMIAL     0x8408
#define CRC_16_L_SEED           0xFFFF
#define CRC_16_L_STEP_SEED      ((uint16_t) ~((uint16_t) CRC_16_L_SEED))
#define CRC_16_L_OK             0x0F47
#define CRC_TAB_SIZE            256

const uint16_t crc_16_l_table[ CRC_TAB_SIZE ] = {
  0x0000, 0x1189, 0x2312, 0x329b, 0x4624, 0x57ad, 0x6536, 0x74bf,
  0x8c48, 0x9dc1, 0xaf5a, 0xbed3, 0xca6c, 0xdbe5, 0xe97e, 0xf8f7,
  0x1081, 0x0108, 0x3393, 0x221a, 0x56a5, 0x472c, 0x75b7, 0x643e,
  0x9cc9, 0x8d40, 0xbfdb, 0xae52, 0xdaed, 0xcb64, 0xf9ff, 0xe876,
  0x2102, 0x308b, 0x0210, 0x1399, 0x6726, 0x76af, 0x4434, 0x55bd,
  0xad4a, 0xbcc3, 0x8e58, 0x9fd1, 0xeb6e, 0xfae7, 0xc87c, 0xd9f5,
  0x3183, 0x200a, 0x1291, 0x0318, 0x77a7, 0x662e, 0x54b5, 0x453c,
  0xbdcb, 0xac42, 0x9ed9, 0x8f50, 0xfbef, 0xea66, 0xd8fd, 0xc974,
  0x4204, 0x538d, 0x6116, 0x709f, 0x0420, 0x15a9, 0x2732, 0x36bb,
  0xce4c, 0xdfc5, 0xed5e, 0xfcd7, 0x8868, 0x99e1, 0xab7a, 0xbaf3,
  0x5285, 0x430c, 0x7197, 0x601e, 0x14a1, 0x0528, 0x37b3, 0x263a,
  0xdecd, 0xcf44, 0xfddf, 0xec56, 0x98e9, 0x8960, 0xbbfb, 0xaa72,
  0x6306, 0x728f, 0x4014, 0x519d, 0x2522, 0x34ab, 0x0630, 0x17b9,
  0xef4e, 0xfec7, 0xcc5c, 0xddd5, 0xa96a, 0xb8e3, 0x8a78, 0x9bf1,
  0x7387, 0x620e, 0x5095, 0x411c, 0x35a3, 0x242a, 0x16b1, 0x0738,
  0xffcf, 0xee46, 0xdcdd, 0xcd54, 0xb9eb, 0xa862, 0x9af9, 0x8b70,
  0x8408, 0x9581, 0xa71a, 0xb693, 0xc22c, 0xd3a5, 0xe13e, 0xf0b7,
  0x0840, 0x19c9, 0x2b52, 0x3adb, 0x4e64, 0x5fed, 0x6d76, 0x7cff,
  0x9489, 0x8500, 0xb79b, 0xa612, 0xd2ad, 0xc324, 0xf1bf, 0xe036,
  0x18c1, 0x0948, 0x3bd3, 0x2a5a, 0x5ee5, 0x4f6c, 0x7df7, 0x6c7e,
  0xa50a, 0xb483, 0x8618, 0x9791, 0xe32e, 0xf2a7, 0xc03c, 0xd1b5,
  0x2942, 0x38cb, 0x0a50, 0x1bd9, 0x6f66, 0x7eef, 0x4c74, 0x5dfd,
  0xb58b, 0xa402, 0x9699, 0x8710, 0xf3af, 0xe226, 0xd0bd, 0xc134,
  0x39c3, 0x284a, 0x1ad1, 0x0b58, 0x7fe7, 0x6e6e, 0x5cf5, 0x4d7c,
  0xc60c, 0xd785, 0xe51e, 0xf497, 0x8028, 0x91a1, 0xa33a, 0xb2b3,
  0x4a44, 0x5bcd, 0x6956, 0x78df, 0x0c60, 0x1de9, 0x2f72, 0x3efb,
  0xd68d, 0xc704, 0xf59f, 0xe416, 0x90a9, 0x8120, 0xb3bb, 0xa232,
  0x5ac5, 0x4b4c, 0x79d7, 0x685e, 0x1ce1, 0x0d68, 0x3ff3, 0x2e7a,
  0xe70e, 0xf687, 0xc41c, 0xd595, 0xa12a, 0xb0a3, 0x8238, 0x93b1,
  0x6b46, 0x7acf, 0x4854, 0x59dd, 0x2d62, 0x3ceb, 0x0e70, 0x1ff9,
  0xf78f, 0xe606, 0xd49d, 0xc514, 0xb1ab, 0xa022, 0x92b9, 0x8330,
  0x7bc7, 0x6a4e, 0x58d5, 0x495c, 0x3de3, 0x2c6a, 0x1ef1, 0x0f78
};

uint16_t crc_16_l_step(uint16_t seed, const void* buf_ptr, uint32_t byte_len)
{
  uint16_t crc_16;
  uint8_t *byte_ptr = (uint8_t *) buf_ptr;

  for (crc_16 = ~seed ; byte_len > 0; byte_len--, byte_ptr++) {
    crc_16 = crc_16_l_table[ (crc_16 ^ *byte_ptr) & 0x00ff ] ^ (crc_16 >> 8);
  }

  return( ~crc_16 );

}

uint16_t crc_16_l_calc(uint8_t* buf_ptr, uint32_t len)
{
  uint16_t data, crc_16;

  for (crc_16 = CRC_16_L_SEED ; len >= 8; len -= 8, buf_ptr++) {
    crc_16 = crc_16_l_table[ (crc_16 ^ *buf_ptr) & 0x00ff ] ^ (crc_16 >> 8);
  }

  if (len != 0) {

    data = ((uint16_t) (*buf_ptr)) << 8;

    while (len-- != 0)
    {
      if ( ((crc_16 ^ data) & 0x01) != 0)
      {
        crc_16 >>= 1;
        crc_16 ^= CRC_16_L_POLYNOMIAL;
      } else {
        crc_16 >>= 1;
      }
      data >>= 1;
    }
  }

  return( ~crc_16 );

}

/* Packet */

struct incoming_data packet_data[NUMBER_OF_PACKETS];

/* We need 4 bytes extra for adjustment purposes. */
uint8_t aligned_buffer[MAX_DATA_LENGTH + 4]  __attribute__((aligned(4)));

/* Lists. */
incoming_t free_list;           /* Unused buffers. */
incoming_t waiting;             /* Waiting to be processed. */
incoming_t waiting_last;        /* Last waiting packet (for queuing). */
incoming_t current;             /* The current packet for incoming
                                   data. */
/* Set to true after an escape character. */
static uint8_t escape_state = 0;

/* True indicates that we need our hello packet before seeing
   anything. */
uint8_t need_hello = 1;

/* Reply packet. */
uint8_t reply_buffer[REPLY_BUFFER_SIZE];
uint16_t reply_length;

/* Flag to indicate active packet processing, only used by USB */
bool process_packet_flag = false;

static uint32_t bytes_recd = 0;

void DRL_Packet_SendErrorCodeText(uint16_t e, const char* s);
void DRL_Packet_SendErrorText(const char* s);

extern int (*DRL_Packet_Cmds[]) (incoming_t);

void DRL_Packet_Setup(void)
{
  int i;
  packet_data[0].next = NULL;
  packet_data[0].length = 0;

  /* Initialize free list. */
  for (i = 1; i < NUMBER_OF_PACKETS; i++)
  {
    packet_data[i].next = &packet_data[i - 1];
    packet_data[i].length = 0;
  }

  free_list = &packet_data[NUMBER_OF_PACKETS - 1];
  waiting = 0;
  current = 0;

  escape_state = 0;
  waiting_last = 0;
  need_hello = 1;
}

void DRL_Packet_ProcCurrent(void)
{
  /* Begin with some simple sanity checks. */
  if (current->length > MAX_PACKET_SIZE || current->length < 3)
  {
    current->length = 0;
    return;
  }

  /* Queue it up to be processed. */
  if (waiting_last == NULL)
  {
    waiting = waiting_last = current;
    current = NULL;
  }
  else
  {
    waiting_last->next = current;
    waiting_last = current;
    current = NULL;
  }
}


void DRL_Packet_RX(uint8_t ch)
{
  bytes_recd++;

  if (current == NULL)
  {
    if (free_list == NULL)
    {
      DRL_Packet_SendErrorText (DRL_MSG_WIN_OVERRUN);
      return;
    }

    current = free_list;
    free_list = free_list->next;

    current->length = 0;
    current->next = NULL;
  }

  switch (ch)
  {
    case 0x7E:
      DRL_Packet_ProcCurrent();
      escape_state = 0;
      break;

    case 0x7D:
      escape_state = 1;
      break;

    default:
      if (current->length < MAX_PACKET_SIZE)
      {
        if (escape_state)
        {
          ch ^= 0x20;
          escape_state = 0;
        }

        current->buffer[current->length] = ch;
      }

      /* Increment the count anyway.  We will automatically discard
         packets that are too long. */
      current->length++;
  }
}

void DRL_Packet_TX()
{
  int i;
  int ch;
#ifdef PLATFORM_USES_USB
  int j;

  j = 0;
#endif

  CHECK_FOR_DATA ();

  /* Since we don't know how long it's been. */
  TRANSMIT_BYTE (0x7E);
#ifdef PLATFORM_USES_USB
  j++;
#endif

  for (i = 0; i < reply_length; i++)
  {
    /* we only need to check once every 31 characters, since RX and TX
     * run at about the same speed, and our RX FIFO is 64 characters
     */
    if ((i & 31) == 31)
      CHECK_FOR_DATA ();

    ch = reply_buffer[i];

    if (ch == 0x7E || ch == 0x7D)
    {
      TRANSMIT_BYTE (0x7D);
      TRANSMIT_BYTE (0x20 ^ ch);
#ifdef PLATFORM_USES_USB
      j += 2;
#endif
    }
    else
    {
      TRANSMIT_BYTE (ch);
#ifdef PLATFORM_USES_USB
      j++;
#endif
    }
  }

  CHECK_FOR_DATA ();
  TRANSMIT_BYTE (0x7E);
#ifdef PLATFORM_USES_USB
  j++;
#endif

#ifdef PLATFORM_USES_USB
  /* Hack for USB protocol.  If we have an exact multiple of the USB frame
   * size, then the last frame will not be sent out.  The USB standard says
   * that a "short packet" needs to be sent to flush the data.  Two flag
   * characters can serve as the short packet.  Doing it this way, we only
   * perform this test once on every entire packet from the target, so the 
   * over head is not too much.
   */
  if ((j%64) == 0)
  {
    TRANSMIT_BYTE (0x7E);
    TRANSMIT_BYTE (0x7E);
  }
#endif 
}

void DRL_Packet_CalcCRC16()
{
  uint16_t crc = crc_16_l_calc (reply_buffer, reply_length * 8);
  reply_buffer[reply_length] = crc & 0xFF;
  reply_buffer[reply_length + 1] = crc >> 8;

  reply_length += 2;
}

void DRL_Packet_SendErrorText(const char* s)
{
  size_t s_len = strlen(s);

  reply_buffer[0] = DRL_CMD_ERROR_STRING_W;
  reply_buffer[1] = s_len&0xff;
  reply_buffer[2] = (s_len>>8)&0xff;
  memcpy(reply_buffer+3, s, s_len);

  reply_length = 3+s_len;
  DRL_Packet_CalcCRC16();
  DRL_Packet_TX();
}

void DRL_Packet_SendErrorCodeText(uint16_t e, const char* s)
{
  size_t s_len = strlen(s);

  reply_buffer[0] = DRL_CMD_ERROR_CODE_STRING_W;
  reply_buffer[1] = e&0xff;
  reply_buffer[1] = (e>>8)&0xff;
  reply_buffer[3] = s_len&0xff;
  reply_buffer[4] = (s_len>>8)&0xff;
  memcpy(reply_buffer+5, s, s_len);

  reply_length = 5+s_len;
  DRL_Packet_CalcCRC16();
  DRL_Packet_TX();
}

int DRL_Packet_SendInvalid(incoming_t i)
{
  DRL_Packet_SendErrorCodeText(DRL_ERR_INVALID, DRL_MSG_INV);
  return 0;
}

extern bool DRL_Flash_Detected; //flash detected flag!

void DRL_Packet_Proc()
{
  incoming_t this;
  int command, pos, count;
  uint16_t crc;

  CHECK_FOR_DATA ();

  while (waiting != NULL)
  {
    /* Enable flag, only used by USB */
    process_packet_flag = true;
    
    /* Pull it out of the waiting queue. */
    this = waiting;
    waiting = waiting->next;
    if (waiting == NULL)
      waiting_last = NULL;
    this->next = NULL;

    /* Need to verify the CRC.  Unfortunately, the time to compute the
       CRC of the buffer when running out of ram is longer than the
       time to fill up the FIFO. */
    crc = CRC_16_L_STEP_SEED;
    pos = 0;
    while (pos < this->length)
    {
      CHECK_FOR_DATA();
      count = this->length - pos;
      if (count > 128)
        count = 128;
      crc = crc_16_l_step (crc, this->buffer + pos, count);
      pos += count;
    }
    
    if (crc != CRC_16_L_OK)
    {
      DRL_Packet_SendErrorText(DRL_MSG_CHECKSUM);
      goto bogus_packet;
    }
    else
    {
      this->length -= 2;
    }

    command = this->buffer[0];

    if (command >= DRL_CMD_STARTING_CMD && command <= DRL_CMD_ENDING_CMD
        && (!need_hello || command == 1))
    {
      if (DRL_Flash_Detected == false)
      {
        DRL_Packet_SendErrorCodeText(DRL_ERR_FAILED_ID, DRL_MSG_FAILED_ID);
      }
      else
      {
        (*DRL_Packet_Cmds[command])(this);
      }
    }
    else
    {
      DRL_Packet_SendInvalid(this);
    }

  bogus_packet:
    /* Add back to free list. */
    this->next = free_list;
    free_list = this;
  }

  process_packet_flag = false;
}
